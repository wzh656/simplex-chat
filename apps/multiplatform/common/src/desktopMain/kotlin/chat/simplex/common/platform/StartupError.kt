...
// icon and a message above a read-only, selectable, scrolling box with the stack trace and an OK button.
fun showStartupError(e: Throwable) {
  ...
  try {
    ...
    showWindowsErrorWindow("Gray Heterotopia failed to start", e.stackTraceToString().lines().joinToString("\r\n"))
  } catch (_: Throwable) {
...
// Shows a modal-style native error dialog: error icon and message, a read-only selectable edit control
// with the stack trace, and an OK button. Blocks on its own
...
private fun showWindowsErrorWindow(title: String, trace: String) {
  ...
  user32.CreateWindowEx(0, "STATIC", "Gray Heterotopia could not start. Copy the error below for diagnostics.",
  ...
  val traceView = user32.CreateWindowEx(WS_EX_CLIENTEDGE, "EDIT", null,
    ...
    18, 60, cw - 36, ch - 60 - 54, window, null, hInstance, null)
  ...
  for (view in listOf(traceView, okButton)) {
